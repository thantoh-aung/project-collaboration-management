...
export default function RatingComponent({ freelancerId, existingRating, onRatingSubmitted }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('/api/freelancer-reviews', { freelancer_id: freelancerId, rating, comment });
            onRatingSubmitted(response.data);
        } catch (error) {
            let msg = 'Failed to submit rating. Please try again.';
            if (error.response?.data?.error) msg = error.response.data.error;
            else if (error.response?.data?.message) msg = error.response.data.message;
            else if (error.response?.status === 403) msg = 'Only clients can rate freelancers.';
    ...
    return (
        <div className="bg-white rounded-xl border border-[#E2E8F0] p-4 shadow-sm">
            <h4 className="font-semibold text-[#0F172A] mb-3">Rate this Freelancer</h4>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Share your experience working with this freelancer..." className="w-full px-3 py-2 bg-white border border-[#E2E8F0] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#4F46E5] text-[#0F172A] placeholder-[#94A3B8]" rows="3" />
                </div>
                <Button type="submit" disabled={isSubmitting || rating === 0} className="w-full bg-[#4F46E5] hover:bg-[#4338CA] text-white">
